...
class MACrossStrategy(BaseStrategy):
    ...
    def __init__(self, strategy_id: int, symbol: str, timeframe: str, params: Dict[str, Any]):
        # Upravíme __init__ tak, aby přijímal i timeframe
        super().__init__(strategy_id, symbol, params)
        self.timeframe = timeframe # Uložíme si timeframe
    ...
    def on_candle(self, candle: pd.Series):
        ...
        # Upravíme logování pro přehlednost
    ...
    def generate_signal(self) -> Optional[Dict[str, Any]]:
        ...
        signal_type = None # Bude 'GO_LONG', 'GO_SHORT'
        ...
        # Pravidla pro generování signálu na základě stavu
        if self.state == 'FLAT':
            if is_bullish_cross:
                signal_type = 'GO_LONG'
                self.state = 'LONG'
            ...
            if is_bearish_cross:
                signal_type = 'GO_SHORT' # Překlopit pozici z long do short
                self.state = 'SHORT'
        
        elif self.state == 'SHORT':
            if is_bullish_cross:
                signal_type = 'GO_LONG' # Překlopit pozici z short do long
                self.state = 'LONG'

        # Aktualizujeme poslední hodnoty pro další svíčku
